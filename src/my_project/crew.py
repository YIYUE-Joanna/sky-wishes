    ...
    if not u_id:
        ...
        if not is_guest:
            ...
            if is_login:
                ...
                # 恢复 Forgot Password 功能，出现在 Sign In 下方
            ...
            # 注册逻辑
            if is_signup:
                if st.button("Create Account" if sel_lang == "English" else "提交注册"):
                    try:
...
# 根据截图，整合所有可用模型，并将 gemini-2.5-flash-lite 设为首位
MODELS_TO_TRY = [
    "gemini-2.5-flash-lite",  # 额度最高 (1000 RPD)
    "gemini-3-flash",         # 截图显示的最新模型
    "gemini-2.5-flash",       # 标准模型 (当前 22/20 已满)
    "gemma-3-27b",            # 截图中的 Gemma 家族
    "gemma-3-12b",
    "gemma-3-4b",
    "gemma-3-2b",
    "gemma-3-1b",
    "gemini-2.0-flash",       # 备选稳定版
    "gemini-2.5-pro"          # 逻辑最强兜底
]
        ...
        with st.spinner(T["loading"]):
            ...
            for model_name in MODELS_TO_TRY:
                try:
                    ...
                    print(f"Model {model_name} failed. Trying next candidate.")
                    continue 
